...
export async function generateCodeExplanation({
  ...
  const { object: explanation } = await generateObject({
    ...
    schema: z.object({
      ...
      keyPoints: z.array(z.string().describe("Key concepts and patterns used in the code")),
      possibleImprovements: z.array(z.string().describe("Potential ways to improve the code")),
    }),
  })
...
export async function generateCodeSuggestion({
  ...
  const { object: suggestion } = await generateObject({
    ...
    schema: z.object({
      ...
      requirements: z.array(z.string().describe("Required dependencies or setup steps")),
    }),
  })
...
export async function generateBugFix({
  ...
  const { object: bugfix } = await generateObject({
    ...
    schema: z.object({
      ...
      preventionTips: z.array(z.string().describe("Tips to prevent similar bugs in the future")),
    }),
  })
...
export async function generateCodeReview({
  ...
  const { object: review } = await generateObject({
    ...
    schema: z.object({
      ...
      strengths: z.array(z.string().describe("Good practices found in the code")),
      improvements: z.array(z.string().describe("Suggested improvements and best practices")),
      securityIssues: z.array(z.string().describe("Potential security concerns")),
      performanceTips: z.array(z.string().describe("Performance optimization suggestions")),
    }),
  })
...
export async function generateTestCases({
  ...
  const { object: tests } = await generateObject({
    ...
    schema: z.object({
      ...
      coverage: z.array(z.string().describe("Areas of code covered by the tests")),
    }),
  })
